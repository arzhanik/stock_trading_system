...
void Controller::operations() {
	...
	while (operation != "exit") {
		book.print_book();
		std::cout << "Enter operation: 1 - 'add stock', 2 - 'add trader', 3 - 'add trader balance', 4 - 'by stock', 5 - 'cell stock', 6 - 'print trader info' or 0 - 'exit': ";
		try {
			...
			if ((operation == "add stock") || (operation == "1")) {
				...
				if (traders.find(name) != traders.end()) {
					...
					if (stocks.find(company) != stocks.end()) {
                                		Stock& stock = *(stocks[company]);
                                		book.change_stock(&stock);
                        		} else {
                                		std::cout << "Unknown company: " << company << std::endl;
                        		}
				} else {
    					std::cout << "Unknown trader: " << name << std::endl;
				}
			} else if ((operation == "cell stock") || (operation == "5"))  {
                        	std::string name;
                        	std::string company;
                        	int count;
                        	std::cout << "Trader name: ";
                        	...
                        	if (traders.find(name) != traders.end()) {
                               		Trader& trader = *(traders[name]);
                                	trader.cell_stock(book, company, count);
					if (stocks.find(company) != stocks.end()) {
                                        	Stock& stock = *(stocks[company]);
                                        	book.change_stock(&stock);
                                	} else {
			...
			} else if ((operation == "print trader info") || (operation == "6")) {
